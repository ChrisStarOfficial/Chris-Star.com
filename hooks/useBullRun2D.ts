 ...
 * - Uses sprite sheet under /games/bull-run-2d
...
export default function useBullRun2D(opts: HookOptions): HookAPI {
  ...
  useEffect(() => {
    ...
    const assetBase = isHiDPIRef.current
      ? "/games/bull-run-2d/default_200_percent"
      : "/games/bull-run-2d/default_100_percent";
    ...
    soundJumpRef.current.src = `/games/bull-run-2d/audio/press.ogg`;
    soundHitRef.current.src = `/games/bull-run-2d/audio/hit.ogg`;
    soundCheckpointRef.current.src = `/games/bull-run-2d/audio/reached.ogg`;
    // --- End Asset Loading ---
